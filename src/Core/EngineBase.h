...
public:
	...
	static const std::string& GetModeName(int mode)
	{
...
protected:
	...
	virtual void GatherInternal(void* pOut,size_t nOut) = 0;
...
private:
	...
	inline void setTimingMode(MODE mode)
	{
		if(mode == _timingMode)
			return;
		...
		_timeByMode[_timingMode] += time - _beginTime;
	...
	volatile u32					_threadCount;
	std::array<u64,7>				_timeByMode;
	u64								_beginTime;
...
private:
	...
	void GatherInternal(void* pOut,size_t nOut)
	{
		_sampleGenerator.gatherOutput(pOut,nOut);
	}
	...
	MODE ChangeMode(bool bTerminate)
	{
		...
		case INTERSECT:
			...
			_sampleData.completeIntersecting();

			assert(!_rayData.hasRaysToIntersect());

			_sampleData.pushStack();
			if(_sampleData.hasIntersectionsToSubsample())
				newMode = SUBSAMPLE;
			else
			{
				// shadow rays only
		...
		case SUBSAMPLE:
			...
			do
			{ 
				if(_sampleData.emptyStack())
				{
					newMode = GENERATE;
					break;
				}
				else if(_sampleData.hasIntersectionsToShade())
				{
					newMode = SHADE;
					break;
				}
		...
		case GENERATE:
		case STARTUP:
			_sampleGenerator.completeGenerating();
			_sampleData.completeGenerating();
			_rayData.completeAllocatingRays();
			if(_rayData.hasRaysToIntersect())
				newMode = INTERSECT;
			else
		...
		case SUBSAMPLE:
			...
			break;
		case INTERSECT:
			_sampleData.prepareIntersecting();
			_rayData.prepareIntersecting();
			break;
		case GENERATE:
			_sampleGenerator.pushCompleted();
		case STARTUP:
			_sampleGenerator.prepareGenerating();
			_sampleData.prepareGenerating();
			_rayData.prepareAllocatingRays();
			break;
		}
